...
export class HelpCommand extends BaseCommand {
    public execute(message: IMessage, args: string[]): void {
        ...
        if (command && !command.meta.disable) {
            message.channel.send(
                new MessageEmbed()
                    ...
                    .addFields({ name: "Name", value: `\`${command.meta.name}\``, inline: true },
                        ...
                        { name: "Aliases", value: `${Number(command.meta.aliases?.length) > 0 ? command.meta.aliases?.map((c: any,) => `\`${c}\``).join(", ") as string : "None."}`, inline: true },
            ...
            message.channel.send(
                createEmbed("info", message.client.commands.filter((cmd: any,) => !cmd.meta.disable && cmd.meta.name !== "eval").map((c: any,) => `\`${c.meta.name}\``).join(" "))
                    .setTitle("Help Menu")
                    .setColor("#00FF00")
                    .setThumbnail(message.client.user?.displayAvatarURL() as string)
                    .setFooter(`Use ${message.client.config.prefix}help <command> to get more info on a specific command!`, "https://raw.githubusercontent.com/error404Unknown/MusicBot.ts/main/info.png")
            ).catch(e => this.client.logger.error("HELP_CMD_ERR:", e));
        }
    }
}
