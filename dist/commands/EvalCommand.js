...
let EvalCommand = class EvalCommand extends BaseCommand_1.BaseCommand {
    async execute(message, args) {
        ...
        if (!client.config.owners.includes(msg.author.id)) {
            return message.channel.send(createEmbed_1.createEmbed("error", "❗ This command is limited to the bot owner only"));
        }
        ...
        try {
            ...
            if (!code)
                return message.channel.send(createEmbed_1.createEmbed("error", "No js code was provided"));
            ...
            if (error.length > 1024) {
                ...
                embed.setColor("#FF0000").addField("Error", `\`\`\`js\n${error}\`\`\``);
            }
    ...
    hastebin(text) {
        return new Promise((resolve, reject) => {
            const req = https_1.request({ hostname: "hastebin.com", path: "/documents", method: "POST", minVersion: "TLSv1.3" }, res => {
                let raw = "";
                res.on("data", chunk => raw += chunk);
                res.on("end", () => {
                    if (res.statusCode >= 200 && res.statusCode < 300)
                        return resolve(`https://hastebin.com/${JSON.parse(raw).key}`);
                    return reject(new Error(`[hastebin] Error while trying to send data to https://hastebin.com/documents, ${res.statusCode} ${res.statusMessage}`));
                });
            }).on("error", reject);
